...
class TrafficDetectionPipeline:
    ...
    def run_preprocessing(self, input_data: str = None):
        ...
        try:
            ...
            # Извлечение признаков через MLEngine
            ml_engine = MLEngine()

            processed_features = []
            for _, row in df.iterrows():
                # Создание TrafficFeatures из строки
                features = TrafficFeatures(
                    ...
                    payload_size=int(row.get('payload_len', 0)),
                    payload_entropy=self._calculate_entropy(row.get('payload_sample', '')),
                    tcp_flags=int(row.get('tcp_flags', 0)) if row.get('tcp_flags') else 0,
                    timestamp=float(row.get('ts_us', 0)) / 1e6,
                    inter_arrival_time=0.0  # Нужно рассчитать
                )

                # Обработка через ML engine
                prediction = ml_engine.process_packet(features)
            ...
            logger.info(f"Предобработка завершена. Сохранено в {output_path}")
            return True
