...
class SyntheticTrafficGenerator:
    ...
    def generate_anomaly_burst(self, base_records: List[Dict[str, Any]], burst_start: int, burst_duration: int = 10, multiplier: int = 5) -> List[Dict[str, Any]]:
        ...
        if self._last_base_time is None and base_records:
            self._last_base_time = base_records[0]["ts_us"] / 1_000_000

        for record in base_records:
            anomalous_records.append(record)

            # Добавление burst в указанное время
            rel_ts = record["ts_us"] / 1_000_000 - float(self._last_base_time or 0.0)
            if burst_start <= rel_ts <= burst_start + burst_duration:
